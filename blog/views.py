...
def index(request):
    ...
    liked_posts = Post.objects.annotate(Count('likes')).order_by('-likes__count').prefetch_related('author')
    most_liked_posts = liked_posts[:5]

    context = {
        'most_popular_posts': [
            serialize_post(post) for post in most_liked_posts
        ],
