...
def index(request):
    ...
    fresh_posts_with_comments = Post.objects.filter(id__in=fresh_posts_ids).annotate(comments_count=Count('comments'))
    fresh_ids_and_comments = fresh_posts_with_comments.values_list('id', 'comments_count')
    count_for_fresh_id = dict(fresh_ids_and_comments)
    for post in fresh_posts:
        post.comments_count = count_for_fresh_id[post.id]
